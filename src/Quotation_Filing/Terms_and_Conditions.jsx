...
export const termsBasedConditions = {
  ...
  CIF: [
    ...
    "Transit insurance will be in the customer's account and OmTrans will not be responsible for any claim.",
...
export const segmentTerms = {
  ...
  "Air Import": [
    "GST 18%  will be applicable on above charges except air freight as per India govt regulations.In case billing to foreign party GST will not apply.",
...
export const allAvailableTerms = [
  ...
  "Above rates are subject to origin storage, inspection, fumigation, COO, and destination duties/taxes.",
].sort();
...
export const getTermsForSegment = (segment, termsOption = null) => {
  ...
  // Air Export specific terms based on selected terms option
  if (segment === "Air Export" && termsOption === "FOB") {
    const specificTerms = segmentTerms[segment] || [];
    return [...commonTerms, ...specificTerms, "Rates are Subject to Origin Locals."];
  }

  if (segment === "Air Export" && termsOption === "DDP") {
    const specificTerms = segmentTerms[segment] || [];
    return [...commonTerms, ...specificTerms, "Above rates are subject to origin storage, COO, destination duty, tax, storage, and inspection."];
  }

  // Air Import specific terms based on selected terms option
  if (segment === "Air Import" && termsOption === "FOB") {
    const specificTerms = segmentTerms[segment] || [];
    return [...commonTerms, ...specificTerms, "Rates are Subject to Origin Locals."];
  }
