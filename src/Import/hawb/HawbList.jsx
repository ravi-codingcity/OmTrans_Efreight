...
const HawbList = ({ currentUser, onNew, onEdit }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((r) =>
      [r.house_awb_number, r.master_awb_number, r.shipper, r.consignee, r.airport_of_destination, r.createdBy]
  ...
  const handleDelete = async (r) => {
    ...
    if (!window.confirm(`Delete HAWB ${r.house_awb_number || ""}? This cannot be undone.`)) return;
    setDeletingId(r._id);
    try {
      await deleteHawb(r._id);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-4 py-3 shadow-sm">
        <div>
          <h2 className="text-sm font-bold text-gray-800 flex items-center gap-2">
            <Plane size={16} className="text-violet-600" /> HAWB Records
          </h2>
          <p className="text-[11px] text-gray-500">All generated House Air Waybill documents</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={fetchRecords} className="flex items-center gap-1 text-xs text-gray-500 hover:text-violet-600 px-2 py-1.5">
            <RefreshCw size={13} /> Refresh
          </button>
          <button onClick={onNew} className="flex items-center gap-1.5 bg-violet-600 text-white text-xs font-semibold px-3 py-1.5 rounded-md hover:bg-violet-700">
            <Plus size={14} /> New HAWB
          </button>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by HAWB/MAWB no, shipper, consignee..." className="w-full pl-9 pr-3 py-2 text-xs border border-gray-300 rounded-lg focus:ring-1 focus:ring-violet-400" />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-10 text-center text-sm text-gray-400">
          No HAWB records yet. Click <span className="font-semibold text-violet-600">New HAWB</span> to create one.
        </div>
      ) : (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200 text-gray-600">
                  ...
                  <th className="px-3 py-2.5 text-left font-semibold">MAWB No</th>
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr key={r._id} className="border-b border-gray-100 hover:bg-violet-50/40">
                    <td className="px-3 py-2.5 font-bold text-violet-700">{r.house_awb_number || "—"}</td>
                    <td className="px-3 py-2.5 text-gray-700">{r.master_awb_number || "—"}</td>
